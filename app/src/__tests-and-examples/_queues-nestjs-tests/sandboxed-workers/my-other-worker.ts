...
export default async function (job: SandboxedJob) {
  await job.log('Start processing OTHER job');

  const message = await timeoutThenMessage(1000, 'fini OTHER !');

  console.log('Doing something OTHER useful...', job.id, job.data, message);
}
